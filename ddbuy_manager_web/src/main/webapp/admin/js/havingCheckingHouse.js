$(function () {
    ...
    $('#dg').datagrid({
        title: "房屋信息",
        url: 'getHavingcheckingHouse',  //服务器地址
        toolbar: "#ToolBar",  //工具栏
        ...
        columns: [[
            ...
            {field:'x',title:'操作', width:80,align:'left',
                formatter: function(value,row,index){
                    return "<a href='javascript:BackHouse("+row.id+")'>打回</a>";
...
//==================================打回js===================================================
function BackHouse(args) {
    $.messager.confirm('提示框', '确认打回吗?', function(r){
        if (r){
            $.post("BackHouse",{"id":args},function (data) {
                if(data.result==1){
                    $("#dg").datagrid("reload");
                }else{
                    $.messager.alert("提示框","打回失败","info");
                }
            },"json");
        }
    });
...
function BackHouses() {
    ...
    $.messager.confirm("提示框","确认打回吗?",function (flag) {
        if(flag){
            var value="";
            ...
            $.post("BackHouses",{"id":value},function (data) {
                if (data.result>0){
                    $("#dg").datagrid("reload");
                }else{
